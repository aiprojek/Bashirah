    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 animate-fade-in">
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-stone-100 flex justify-between items-center bg-stone-50/50">
                <div>
                     <h3 className="text-xl font-bold text-quran-dark font-serif">
                         Info & Asbabun Nuzul
                     </h3>
                     {surah && (
                         <p className="text-sm text-gray-500">QS. {surah.transliteration} ({surah.translation})</p>
                     )}
                </div>
                <button 
                    onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-white">
                {isLoading ? (
                    ...
                    <div className="space-y-6">
                        ...
                        {surah && (
                            <div className="grid grid-cols-3 gap-4 mb-6">
                                <div className="bg-stone-50 p-3 rounded-xl border border-stone-100 text-center">
                                    ...
                                    <div className="font-bold text-quran-dark text-lg">{surah.total_verses}</div>
                                    <div className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Ayat</div>
                                </div>
                                <div className="bg-stone-50 p-3 rounded-xl border border-stone-100 text-center">
                                    <div className="text-gray-400 mb-1 flex justify-center"><MapPin className="w-4 h-4" /></div>
                                    <div className="font-bold text-quran-dark text-lg">{surah.type}</div>
                                    <div className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Tempat</div>
                                </div>
                                <div className="bg-stone-50 p-3 rounded-xl border border-stone-100 text-center">
                                    <div className="text-gray-400 mb-1 flex justify-center"><AlignLeft className="w-4 h-4" /></div>
                                    <div className="font-bold text-quran-dark text-lg">{surah.id}</div>
                                    <div className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Urutan</div>
                                </div>
                            </div>
                        )}

                        {/* Main Text content */}
                        <div className="prose prose-sm md:prose-base max-w-none text-gray-700 leading-relaxed font-serif">
                        ...
                        <div className="mt-8 pt-4 border-t border-stone-100 text-[10px] text-gray-400 italic">
                            Sumber: {info.source}
                        </div>
                    </div>
                ) : (
