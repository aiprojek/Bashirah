        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in min-h-screen pb-24 relative">
            ...
            <div className="flex items-center gap-4 mb-8">
                {(selectedCategory || selectedRule) && (
                    <button onClick={handleBack} className="p-2 bg-stone-100 rounded-full hover:bg-stone-200 transition-colors">
                        <ArrowLeft className="w-5 h-5 text-gray-600" />
                    </button>
                )}
                <div>
                    <h2 className="text-2xl font-bold text-quran-dark font-serif flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {TAJWEED_LEARNING_DATA.map((cat) => (
                        <button 
                            ...
                            className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 hover:border-quran-gold/50 hover:shadow-md transition-all text-left group relative overflow-hidden"
                        >
                            ...
                            <h3 className="text-lg font-bold text-gray-800 mb-2 group-hover:text-quran-gold transition-colors">{cat.title}</h3>
                            <p className="text-sm text-gray-500 leading-relaxed mb-4">{cat.description}</p>
                            
                            <div className="flex items-center text-xs font-bold text-quran-dark uppercase tracking-wider bg-stone-50 w-fit px-3 py-1.5 rounded-lg group-hover:bg-quran-dark group-hover:text-white transition-colors">
            ...
            {selectedCategory && !selectedRule && (
                <div className="animate-fade-in">
                    <div className="bg-quran-gold/10 border border-quran-gold/20 p-5 rounded-2xl mb-6">
                        <h3 className="font-bold text-xl text-quran-dark mb-1">{selectedCategory.title}</h3>
                    ...
                    <div className="space-y-4">
                        {selectedCategory.rules.map((rule) => (
                            <div 
                                ...
                                className="bg-white p-5 rounded-xl border border-stone-200 hover:border-quran-gold hover:shadow-sm cursor-pointer transition-all flex justify-between items-center group"
                            >
                                ...
                                <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-quran-gold" />
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {selectedRule && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-lg max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
                        
                        {/* Header */}
                        <div className="px-6 py-5 border-b border-stone-100 flex justify-between items-start bg-stone-50">
                            <div>
                                <span 
                                ...
                                <h3 className="text-2xl font-bold text-quran-dark font-serif">{selectedRule.name}</h3>
                            </div>
                            <button 
                                onClick={() => setSelectedRule(null)}
                                className="p-2 bg-white rounded-full text-gray-400 hover:text-red-500 shadow-sm border border-stone-100 transition-colors"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar space-y-8">
                            ...
                            <section>
                                ...
                                <p className="text-gray-600 text-sm leading-relaxed bg-stone-50 p-4 rounded-xl border-l-4 border-stone-300">
                            ...
                            {/* Examples */}
                            <section>
                                <h4 className="text-sm font-bold text-gray-800 mb-4 border-b border-stone-100 pb-2">
                                    Contoh Lafadz
                                </h4>
                                <div className="space-y-4">
                                    {selectedRule.examples.map((ex, idx) => (
                                        <div key={idx} className="bg-stone-50 rounded-xl p-4 border border-stone-100 text-center">
                                            <p 
                                                className="font-arabic text-3xl text-quran-dark mb-3 leading-relaxed" 
                                                dir="rtl"
                        ...
                        <div className="p-4 border-t border-stone-100 bg-white text-center">
                            <button 
                                onClick={() => setSelectedRule(null)}
                                className="text-xs font-bold text-gray-400 hover:text-quran-dark uppercase tracking-wider transition-colors"
                            >
                                Tutup Penjelasan
                            </button>
                        </div>
