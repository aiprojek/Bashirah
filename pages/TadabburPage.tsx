...
const TAGS: { id: TadabburTag; label: string; color: string }[] = [
    ...
    { id: 'sabar', label: 'Sabar', color: 'bg-orange-100 text-orange-700' },
    { id: 'istighfar', label: 'Istighfar', color: 'bg-stone-200 text-stone-600' },
    { id: 'doa', label: 'Doa', color: 'bg-blue-100 text-blue-700' },
    { id: 'ibrah', label: 'Ibrah/Pelajaran', color: 'bg-purple-100 text-purple-700' },
    { id: 'umum', label: 'Refleksi Umum', color: 'bg-gray-100 text-gray-700' },
];
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in min-h-screen pb-24 relative">
            ...
            <div className="mb-10 text-center">
                 <div className="inline-block p-4 bg-white rounded-full shadow-sm mb-4 border border-stone-100">
                    <BookHeart className="w-8 h-8 text-quran-dark" />
                 </div>
                 <h2 className="text-3xl font-bold text-quran-dark font-serif mb-2">Jurnal Tadabbur</h2>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-16 bg-white/50 rounded-3xl border border-dashed border-stone-200">
                    <PenTool className="w-10 h-10 text-gray-300 mx-auto mb-3" />
                    <p className="text-gray-400 text-sm">Belum ada catatan tadabbur.</p>
                    <button 
                    ...
                    {entries.map(entry => {
                        ...
                        return (
                            <div 
                                ...
                                className="bg-white p-6 rounded-2xl shadow-sm border border-stone-100 hover:shadow-md transition-all cursor-pointer group relative overflow-hidden"
                            >
                                ...
                                <h3 className="text-xl font-bold text-gray-800 font-serif mb-2 line-clamp-1 group-hover:text-quran-gold transition-colors">
                                    {entry.title}
                                </h3>
                                <p className="text-gray-600 text-sm leading-relaxed line-clamp-3 font-serif opacity-80">
                                    {entry.content}
                                </p>
                            </div>
                        );
            ...
            <button 
                onClick={() => handleOpenEditor()}
                className="fixed bottom-6 right-6 md:right-10 w-14 h-14 bg-quran-dark text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-transform z-40 border-2 border-white"
            >
            ...
            {isEditorOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-[#fcfbf7] w-full max-w-2xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
                        ...
                        <div className="px-6 py-4 border-b border-stone-200 bg-white flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button 
                                    onClick={() => setIsEditorOpen(false)}
                                    className="p-2 text-gray-400 hover:bg-stone-100 rounded-lg transition-colors"
                                >
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
                            <input 
                                ...
                                className="w-full text-2xl md:text-3xl font-serif font-bold bg-transparent border-none focus:ring-0 placeholder-gray-300 text-quran-dark mb-6 p-0"
                                value={title}
                                ...
                                {TAGS.map(tag => (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-full text-xs font-bold transition-all border ${
                                            ...
                                            : 'bg-white text-gray-500 border-stone-200 hover:border-quran-gold/50'
                                        }`}
                                    >
                                        {tag.label}
                                    </button>
                            ...
                            <textarea 
                                className="w-full h-[50vh] bg-transparent border-none focus:ring-0 text-gray-700 font-serif text-lg leading-loose resize-none p-0 placeholder-gray-300"
                        ...
                        <div className="p-4 bg-white border-t border-stone-200 flex justify-end">
                            <button 
                                onClick={handleSave}
                                className="px-6 py-3 bg-quran-dark text-white rounded-xl font-bold flex items-center gap-2 hover:bg-quran-dark/90 shadow-lg"
                            >
