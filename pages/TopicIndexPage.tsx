        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in min-h-screen">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {TOPICS.map((topic) => (
                    <div 
                        ...
                        className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-stone-100 dark:border-slate-700 hover:shadow-md hover:border-quran-gold/30 dark:hover:border-quran-gold/30 transition-all cursor-pointer group relative overflow-hidden"
                    >
                         {/* Background Decor */}
                        <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity transform group-hover:scale-110">
                             {renderIcon(topic.iconName, "w-24 h-24 text-quran-dark dark:text-white")}
                        </div>

                        <div className="relative z-10">
                            <div className="w-12 h-12 rounded-xl bg-stone-50 dark:bg-slate-700 flex items-center justify-center text-quran-dark dark:text-quran-gold mb-4 group-hover:bg-quran-dark group-hover:text-white dark:group-hover:bg-quran-gold dark:group-hover:text-quran-dark transition-colors">
            ...
            {selectedTopic && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
                    ...
                    <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in flex flex-col max-h-[85vh] border border-white/10">
                        {/* Modal Header */}
                        <div className="px-6 py-5 border-b border-stone-100 dark:border-slate-700 flex items-center justify-between bg-stone-50/50 dark:bg-slate-700/50">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-quran-gold/10 rounded-lg text-quran-dark dark:text-quran-gold">
                        ...
                        {/* References List */}
                        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-stone-50/30 dark:bg-slate-900/30 space-y-3">
                            {selectedTopic.references.map((ref, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => handleVerseClick(ref.surahId, ref.verseId)}
                                    className="w-full bg-white dark:bg-slate-700 p-4 rounded-xl border border-stone-200 dark:border-slate-600 shadow-sm hover:border-quran-gold dark:hover:border-quran-gold hover:shadow-md transition-all flex items-center justify-between group text-left"
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="w-8 h-8 rounded-full bg-stone-100 dark:bg-slate-600 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-gray-300 border border-stone-200 dark:border-slate-500">
                                            {idx + 1}
                                        </div>
                                        <div>
